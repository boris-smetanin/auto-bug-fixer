...
const SYSTEM_PROMPT_TEMPLATE = (baseBranch: string, fixBranch: string): string =>
  [
    ...
    '5. After making the change, run the project\'s fast static analysis on the edited files and',
    '   fix any new errors before committing. Use whatever the project provides — TypeScript\'s',
    '   `tsc --noEmit`, Python\'s `mypy`/`pyright`, Go\'s `go vet`, Rust\'s `cargo check`, `eslint`,',
    '   `ruff`, etc. Look for the toolchain in package.json scripts, pyproject.toml, Makefile,',
    '   or similar. This catches type signature mismatches, wrong argument shapes, and obvious',
    '   syntax/import errors that a fix can introduce. DO NOT run the project\'s test suite —',
    '   only the fast static checks. If no static checker is configured for the project, skip this.',
    '',
